...
python main.py --discover --types butcher,wine_store  # Specific business types
...
**Phase 1 - Discovery (`discover.py`):** Searches Serper Maps API across ~130 US cities with keyword queries per business type. Deduplicates by phone number, filters chains (extensive blocklist in `config.py`), applies quality floors (restaurants: 50+ reviews/4.2+ rating; niche: 20+ reviews/4.0+), requires website.
